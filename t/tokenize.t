...
tokenize_fail( q<${yehudi:?not foolish}>, qr{ not [ ] foolish }smx );
...

{
    ...
    sub method_succeeds {
	my ( $method, @args ) = @_;
	my $name = _format_method_args( $method, @args );
	if ( eval { $tt->$method( @args ); 1 } ) {
	    @_ = ( $name );
	    goto &pass;
	} else {
	    @_ = ( "$name failed: $@" );
