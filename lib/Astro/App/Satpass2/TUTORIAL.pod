...
This package was created to provide a flexable way to predict satellite
positions, passes, and visibility. Unfortunately, with flexability comes
...
is not, C<Astro::App::Satpass> uses a home-grown ISO-8601-ish date
parser. All dates specified in the examples will be compatable with both
date parsers.
...
least some of the incompatabilites between the two tools. Since the
intent is to remove this functionality, you may at some time wish to
convert your F<satpass> initialization file to a C<Astro::App::Satpass2>
...
executive mansion as our location.
 ...
 satpass2> delegate spacetrack set direct 1
 satpass2>
 ...
 satpass2> delegate spacetrack set with_name 1
 satpass2>
...
point.
 ...
 satpass2> delegate spacetrack spaceflight -all -effective
 satpass2>
...
word-processing editor will probably not work satisfactorally unless you
set the entire document to a monospace font.
...
specify the date more fledably, as (say) C<'1-Apr-2011 noon'>.
...
during the day, and it defines day as any time after the end of morning
twilight and before the start of evening twilight. These in turn are
defined as the point when the upper limb of the Sun passes above or
...
download the orbital data and predict the flare from the data.
 ...
 satpass2> delegate spacetrack celestrak iridium
 satpass2>
...
the end of morning twilight; C<-day> selects flares from the end of
morning twilight to the beginning of evening twilight, and C<-pm>
selects flares from the beginning of evening twilight until midnight.
...
is a separate subsystem all to itself. So:
 ...
 satpass2> delegate formatter date_format '%a %d-%b-%Y
 satpass2>
 satpass2> # Display the time as 1-12 AM or PM
 satpass2> delegate formatter time_format '%I:%M:%S %p'
 satpass2>
...
C<Astro::App::Satpass2> recognises an end-of-line back slash as a
...
Note that the C<"flare"> did not need to be quoted, but it does no harm
to do so, and it makes the documentation look nicer.

Then when you say

 satpass2> iridium_flare

The C<twilight> and C<flare_mag_night> settings will be changed, the
flare prediction will be run, and the old C<twilight> and
C<flare_mag_night> settings will be restored.
