...
C<Astro::App::Satpass2> as though it were an interactive method. In the
following discussion, some words are used with particular meanings:
...
However, calling C<dispatch()> inside your code macro is unsupported.
This does not mean it is forbidden; rather it means that the author does
not intend to provide any particular behavior when this is done, and
whatever the behavior actually is he reserves the right to change it
without notice of any sort at any time.
