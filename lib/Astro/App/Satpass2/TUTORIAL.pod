...
The formatter object also has a C<tz> setting, but this is unsupported
because it relies on the C<TZ> environment variable, and the author has
no control over whether your OS' POSIX code supports this. You can try
it with something like

 satpass2> formatter tz MST7MDT

(for Mountan time). If it works, fine. If not, you can make it work by
installing C<DateTime|DateTime> and
C<DateTime::TimeZone|DateTime::TimeZone>. Doing this also gives you the
...
The C<time_parser> and C<formatter> time zones are set seperately not
...
For example, to make C<azel> report azimuth first,
 ...
 satpass2> tell formatter template azel <<'EOD'
...
documentation for what format effectors are available. The C<data>
