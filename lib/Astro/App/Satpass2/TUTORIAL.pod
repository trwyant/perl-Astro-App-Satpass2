...
attribute to the appropriate value. You should see the documentation for
full details, but with the C<satpass2> script the configuration would
look something like this

 satpass2> # Configure the ISO8601 parser, with reform date
 satpass2> set time_parser ISO8601,reform_date=dflt

The C<'dflt'> specifies the default date, which is the date it was first
adopted in Italy, on October 15 1582 (Gregorian). Any specification
acceptable to 
L<DateTime::Calendar::Christian|DateTime::Calendar::Christian> will work
(e.g. C<'UK'>, which specifies September 24 1752), as will an ISO-8601
time with delimiters (e.g. C<'1752-9-24'>). Specifying any false value
causes all dates to be parsed as Gregorian.
...
the reform date. Because the date formatter is managed by the formatter
object, the C<satpass2> commands to configure it are

 satpass2> # Configure the time formatter and reform date
 satpass2> formatter time_formatter DateTime::Strftime
 satpass2> formatter reform_date dflt

The available values for C<reform_date> are the same as for the parser.
...
C<format_strftime()> method. You may find that it works, because this
module patches in C<format_strftime()> when it loads
L<DateTime::Calendar::Christian|DateTime::Calendar::Christian> if it
finds that that method does not exist. But you may find that it does not
work, because the patch messes with the internals of
L<DateTime::Calendar::Christian|DateTime::Calendar::Christian>, and
