...
EOD
    ...
    alias	=> <<'EOD',
[% DEFAULT data = sp.alias( arg.dereference ) -%]
...
EOD
    ...
    almanac	=> <<'EOD',
[% DEFAULT data = sp.almanac( arg.dereference ) -%]
...
EOD
    ...
    flare => <<'EOD',
[% DEFAULT data = sp.flare( arg.dereference ) -%]
[% IF title %]
    [%- title.time( '' ) %]
        [%= title.name( '', width = 12 ) %]
        [%= title.local_coord( '' ) %]
...
EOD
    ...
    list => <<'EOD',
[% DEFAULT data = sp.list( arg.dereference ) -%]
[% IF title %]
...
EOD
    ...
    location	=> <<'EOD',
[% DEFAULT data = sp.location( arg.dereference ) -%]
...
EOD
    ...
    pass	=> <<'EOD',
[% DEFAULT data = sp.pass( arg.dereference ) -%]
[% IF title %]
...
EOD
    ...
    phase => <<'EOD',
[% DEFAULT data = sp.phase( arg.dereference ) -%]
[% IF title %]
...
EOD
    ...
    position => <<'EOD',
[% DEFAULT data = sp.position( arg.dereference ) -%]
[%- data.date %] [% data.time %]
[% IF title %]
...
EOD
    ...
    tle	=> <<'EOD',
[% DEFAULT data = sp.tle( arg.dereference ) -%]
[% FOR item IN data %]
    [%- item.tle -%]
[% END -%]
EOD
...
sub report {
	...
	local $Template::Stash::LIST_OPS->{dereference} = sub {
	    my ( $arg ) = @_;
	    'ARRAY' eq ref $arg and return @{ $arg };
	    return;
	};
...
All formatters define the following list virtual methods in addition to
the ones provided by L<Template-Toolkit|Template::Manual::VMethods>:

=over

=item dereference

This virtual method expands an array reference into an array. This is
used for passing arguments to external code.

=item events

This virtual method extracts all the events from a list of pass data,
and returns them sorted by date. It silently ignores any list elements
that are not hash references, or do not have an C<{elements}> key.
