...
sub alias {
    my ( $self, $hash ) = @_;
    $hash or $self->warner()->wail( 'Argument is required' );
    my $output;
...
sub list {
    my ( $self, $array ) = @_;
    $array or $self->warner()->wail( 'Argument is required' );
    my $output = $self->_format_execute( header => 'list_inertial' );
    foreach my $body ( @{ $array } ) {
	$output .= $self->_set( body => $body )->_format_execute(
	    format => $body->get( 'inertial' ) ?
	    'list_inertial' :
	    'list_fixed'
	);
    }
...
sub location {
    my ( $self, $station ) = @_;
    $station or $self->warner()->wail( 'Argument is required' );
    return (
...
##	[&PASS_EVENT_NONE => ''],
	...
	foreach my $pass ( @{ $array } ) {
		...
		$output .= $self->_format_execute(
		    format => $event_map[ $event->{event} ], 'pass' );
	    }
	    defined $self->{template}{pass_finish}
...
sub phase {
    my ( $self, $array ) = @_;
    $array or $self->warner()->wail( 'Argument is required' );
    my $output = $self->_format_execute( header => 'phase' );
    foreach my $body ( @{ $array } ) {
	$output .= $self->_set(
	    body => $body, time => $body->universal() )
...
sub position {
    ...
    my $output =
	$self->_format_execute( format => 'date_time' ) .
	$self->_format_execute( header => 'position' );

    foreach my $body ( @{ $hash->{bodies} } ) {
	$self->_set( body => $body, station => $sta );
...
sub tle {
    ...
    my $output;
    foreach my $body ( @{ $array } ) {
	$output .= $self->_set( body => $body )
	    ->_format_execute( format => 'tle' );
    }
    return $output;
}
...
sub _tle_celestia {	## no critic (ProhibitUnusedPrivateSubroutines)
    ...
    foreach my $body ( @{ $array } ) {
	$output .= $self->_set( body => $body )
	->_format_execute( format => 'tle_celestia' );
    }
    return $output;
}
...
sub tle_verbose {
    ...
    foreach my $body ( @{ $array } ) {
	$output .= $self->_set( body => $body )
	->_format_execute( format => 'tle_verbose' );
    }
    return $output;
}
...
the same function. It uses no template, but simply displays the
key/value pairs as
...
same function. It uses template C<list_inertal>, which defaults to
...
is used at the start of the pass, to identify the satellite.  If you use
C<%date> or C<%time> in this template, you get the time the satellite
rises.

Template C<pass_finish> is displayed at the end of each pass. If you use
C<%date> or C<%time> in this template, you get the time the satellite
sets.
...
This method is not inherited from
L<Astro::App::Satpass2::Format|Astro::App::Satpass2::Format>. It alters
defaults for the given format effector. The altered defaults apply only
to the formatter object the method is called on. The $name argument
specifies the format effector to change (no abbreviations allowed here),
and the $attr => $value pairs specify the new defaults.
