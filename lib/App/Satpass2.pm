...

{
	...
	if ( my $cmd = $self->get( 'webcmd' ) ) {
	    $self->system( $cmd,
		"http://search.cpan.org/~wyant/App-Satpass2-$VERSION/");
	} else {
	    $arg = $arg ? lc $arg : '';
	    my @ha;
...
http://search.cpan.org/dist/App-Satpass2/
...
sub _set_formatter {
    my ( $self, $name, $val ) = @_;
    defined $val or $val = 'App::Satpass2::Format::Classic';
...
sub _set_time_parser {
	...
	or $val = 'App::Satpass2::ParseTime';
    return $self->_set_stringable( $name, $val,
	'App::Satpass2::ParseTime', "Unknown time parser '$val'" );
}
...

{
	...
	return ( $dumper ||= do {
		require App::Satpass2::Format::Dump;
		App::Satpass2::Format::Dump->new();
	    }
	);
    }
...
#
#	Gets the App::Satpass2::Format object. If $opt->{dump} is true,
...
App::Satpass2 - Forecast satellite visibility.

=head1 SYNOPSIS

 use App::Satpass2;
 # Instantiate and set our location
 my $satpass2 = App::Satpass2->new(
...
A few methods are used for manipulating the C<App::Satpass2> object
...
C<Data::Dumper>) and returns a dump of the C<App::Satpass2> object.
...
This method can also be called on an App::Satpass2 object. For example:
...
This method saves all attribute values of the C<App::Satpass2> object,
...
The following options are allowed on the 'get' or 'show' commands:
 ...
 -changes reports only changes from the defaults used by App::Satpass2.
...
App::Satpass2, there is no shell processing, and the quoting rules are
those of App::Satpass2, not those of the underlying operating system.
...
The App::Satpass2 object has a number of attributes to configure its
...
L<parse_time|App::Satpass2::ParseTime/parse> method. Since that expects
to parse a string of some sort, you can not (unfortunately) pass in a
Perl time. See the L<App::Satpass2::ParseTime|App::Satpass2::ParseTime>
...
L<App::Satpass2::Format|App::Satpass2::Format>, it C<must> conform to
that class' interface.

The default is C<'App::Satpass2::Format::Classic'>.
...
L<App::Satpass2::Format local_coord|App::Satpass2::Format/local_coord>
documentation for the default. See the documentation of the actual
formatter class being used for what it does.
...
L<perltime|App::Satpass2::ParseTime/perltime> documentation for the
...
The default is C<'App::Satpass2::ParseTime'>, which actually returns one
...
L<App::Satpass2::ParseTime|App::Satpass2::ParseTime> modules for
...
functionality will (almost always) go in C<App::Satpass2>, and changes
...
from a F<satpass> initialization file (as opposed to an C<App::Satpass2>
